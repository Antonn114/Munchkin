...
void init_attack_rays_positive(){
  ...
  for (int f=0; f < 8; f++, east = eastOne(east)){
    U64 ea = east;
    for (int r8 = 0; r8 < 64; r8 += 8, ea <<= 8)
      east_attacks[r8+f] = ea;
  }
}
...
void init_magic(){
  for (int i = 0; i < 64; i++){
    ...
    bishop_magic[i].mask = bishop_attacks[i] & (UNIVERSE ^ (A_FILE | H_FILE | F_RANK | E_RANK));
